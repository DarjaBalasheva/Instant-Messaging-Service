...
from app.models.user import Token, UserLoginModel, UserModel, UserRegisterModel
...
async def register_user(
    user: UserRegisterModel, db_session: AsyncSession = Depends(get_db_session)
):
    try:
...
async def login(
    user_login: UserLoginModel, db_session: AsyncSession = Depends(get_db_session)
):
    try:
